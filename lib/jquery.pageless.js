...
(function ($, window) {
  ...
  function PageLess(element, options) {
    var $element = $(element),
        obj = this,
        isLoading = false,
    ...
    function init() {
      ...
      // loader element
      var $loader = $(options.loader, $element);
      if (options.loader && $loader.length) {
        loader = $loader;
      } else {
        loader = $(loaderHtml());
        ...
        if (!options.loaderHtml) {
          $('#pageless-loader .msg').html(options.loaderMsg).css(options.msgStyles || {});
        }
      }
    };
    ...
    function loading(bool) {
      ...
      if (loader) {
        if (isLoading) {
          loader.show(); // Seedrs fix: no effects
        } else {
          loader.hide(); // Seedrs fix: no effects
        }
      }
    };
    ...
    this.startListener = function() {
      init();
      $(document).on(SCROLL + ' ' + RESIZE, obj.watch);
      // * trigger once in case of reload
      this.watch();
    };
    
    this.stopListener = function() {
      $(document).off(SCROLL + ' ' + RESIZE, obj.watch);
    };
    ...
    this.watch = function() {
      ...
      if (!isLoading && (distanceToBottom() < settings.distance) && ($element.is(":visible"))) {
  ...
  $.fn.pageless = function (options) {
    return this.each(function(){
      var $element = $(this);
      
      // Return early if this element already has a plugin instance
      if ($element.data('pageless')) return $element;
      
      // pass options to plugin constructor
      var pageless = new PageLess(this, options);
      
      pageless.startListener();
      
      // Store plugin object in this element's data
      $element.data('pageless', pageless);
    });
  };
})(jQuery, window);
